...
st.write("Splitting dataset into train and test at the beginning is a good practice, which prevents leaking information and overfitting.")
...
st.write(f"- Stroke cases in y_train: {round(y_train.value_counts()[1]/len(y_train),3)*100}%")
st.write(f"- Stroke cases in y_test: {round(y_test.value_counts()[1]/len(y_test),3)*100}%")

#Create 3 buttons to navigate between 'Categorical Variable Encoding', 'Handling Outliers', 'Deal with Missing Values'
selected = option_menu(
    menu_title = None,
    options = ['Categorical Variable Encoding', 'Handling Outliers', 'Deal with Missing Values'],
...
if selected == 'Categorical Variable Encoding':
    ...
    st.write("**One Hot Encoding** to capture categorical label information for better visualization presentation.")

    st.write("One Hot Encoding will apply to the following categorical feature variables:")
    st.write("- gender")
...
if selected == 'Handling Outliers':
    ...
    st.write('Replace outliers with maximum allowed values with **Capping** method. Replace outliers with median/mean could lose the representation of the population with large BMI in the model.')
   
    st.write("Note: When doing capping, we tend to cap values both in train and test set. It is important to remember that the capping values MUST be derived from the train set. And then use those same values to cap the variables in the test set.")
    ...
    st.write(f'Based on the following clinical consideration, the upper bound is **{ulbmi1}**, with **{bmi_outlier}** outliers.')
    ...
    st.write('After replacing outliers with the upper limit defined previously, there are no more outliers.')
    ...
    st.write('**Binning** helps handle outliers by placing these values into the lower or higher intervals. It allows the right-tail dense group to be represented in the general population.')
    ...
    st.write('The grouped avg_glucose_level is now a categorical variable, which also needs to be encoded as numerical representation.')
...
if selected == 'Deal with Missing Values':
    ...
    st.write('In summary, univariate imputation (median/mean) and multivariate imputation (KNN) return approximately the same performance, with training ROC-AUC score around 97%.')
    st.write('Choose univariate imputation to avoid additional complexity of training models to impute NA.')

    st.write('Note: The imputation values (that is the median/mean) should be calculated using the training set, and the same value should be used to impute the test set. This is to avoid overfitting.')
    ...
    st.write('Median imputation has variable variance slightly closer to the original variance:')
